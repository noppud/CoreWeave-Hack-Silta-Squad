...
class Center:
    ...
    def capture_frame(self):
        """Read the actual Fusion viewport without changing its camera or document."""
        from .fusion import FusionBridge
        ...
        with self.frame_lock:
            ...
            temporary = folder / "fusion-frame-next.png"
            source = (
                "if not app.activeDocument:\n"
                "    raise RuntimeError('No active Fusion document')\n"
                "result = {'document': app.activeDocument.name, "
                "'saved': app.activeViewport.saveAsImageFile(payload['path'], 1280, 800)}\n"
            )
            try:
                reply = FusionBridge().request(
                    "run_script",
                    {"source": source, "arguments": {"path": str(temporary)}},
                    timeout=5,
                )
            except (RuntimeError, TimeoutError, OSError):
                raise ValueError(
                    "Fusion viewport unavailable. Open Fusion and start SiltaBridge."
                ) from None
            result = reply.get("result", {})
            if result.get("saved") is not True or not temporary.is_file():
                raise ValueError("Fusion did not return a viewport image")
            path = folder / "fusion-frame.png"
            temporary.replace(path)
            return dict(
                url=self.catalog.asset(path),
                at=now(),
                document=result["document"],
                scope="Actual Fusion viewport snapshot; not a verification verdict",
            )
...
def make_handler(center):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            elif path in {"/", "/index.html", "/app.js", "/style.css", "/model.js"}:
