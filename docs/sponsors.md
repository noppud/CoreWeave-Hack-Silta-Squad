...
| W&B MCP server | Let a coding agent inspect runs, traces, evaluations, and documentation | Hosted endpoint uses the W&B API key. No separate credit grant stated. | Installed at user scope and reporting connected; restart the client to expose its tools. |
...
Use `silta` for `WANDB_ENTITY`. Verified September 12 against the API key in use: the key's
default entity is `silta`, and the account belongs to both `silta` and `konstav-control-dev`.
Only `silta` is accepted by W&B Inference — sending `konstav-control-dev` in the
`OpenAI-Project` header returns `HTTP 401 invalid_api_key`, which reads like a bad key but is
an entity mismatch. The project is `coreweave-hack-silta-squad`, confirmed live at
[the Weave project](https://wandb.ai/silta/coreweave-hack-silta-squad/weave).
...
- Scope your queries to `silta/coreweave-hack-silta-squad`.
...
- [x] API key configured locally for the verified team (`silta`).
- [ ] Credit request completed and organizer activation confirmed.
- [x] Model catalog fetched; model selected (`deepseek-ai/DeepSeek-V4-Flash`, changeable).
- [x] Synthetic Weave trace sent and flushed; open the project link to confirm it renders.
