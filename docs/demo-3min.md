...
| 4. Outer review loop | 20 s | Shrink the demo and reveal the planned system → ARIA → system loop. |
| 5. Videos | 40 s | Show actual CAD/CAM generation and simulation in the product. |
| 6. Architecture / Weave | 20 s | Explain what Astra, Fusion and Weave do. |
...
| Creativity | Slides 3 and 6: planning, checking and judging roles cooperate through concrete feedback. |
| Utility | Slide 2: one manufacturing problem, stated briefly. |
| Technical execution | Slide 5: actual product recordings. Slide 6: understandable application/Fusion/Weave boundaries. |
| Sponsor usage | Slide 6: trace evidence. Slide 4: planned ARIA review loop; describe the completed advisory work verbally. |
...
**Outer review loop:** Press Next after slide 3. The demo placeholder shrinks to half its width and height, and curved arrows reveal a proposed overarching review cycle. The whole agent system sends traces and outcomes to ARIA; recommendations return to the system. This is a planned design, not a claim that continuous ARIA monitoring or automatic implementation is running. Replace the rectangle with the same system image/video used in the demo when ready. Reduced-motion settings disable the shrink animation.
...
- `agent-loop-overview-slot` on slide 4: smaller view of the whole system, with the planned ARIA loop around it.
- `product-video-1-slot` and `product-video-2-slot` on slide 5: product recordings.
- Slide 6: replace the pending footer with verified Weave evidence after tomorrow’s integration.
