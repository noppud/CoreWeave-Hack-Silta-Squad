...
def _inspect(app):
        ...
        result["bodies"] = [
            {
                ...
                "volume_cm3": b.volume,
                "bounding_box_cm": {
                    "min": b.boundingBox.minPoint.asArray(),
    ...
    if cam:
        ...
        result["nc_programs"] = [
            {"name": p.name, "index": i} for i, p in enumerate(_items(cam.ncPrograms))
        ]
    return result
...
def dispatch(app, action, payload):
    ...
    if action == "generation_status":
        ...
        return {
            "completed": done,
            "result": {"generation_completed": done},
    ...
    if action == "machining_time":
        ...
        time = cam.getMachiningTime(
            collection,
            float(payload["feed_scale_percent"]),
            float(payload["rapid_feed_cm_s"]),
            float(payload["tool_change_seconds"]),
        )
        metrics = {
            "machining_seconds": time.machiningTime,
            "feed_seconds": time.totalFeedTime,
            "rapid_seconds": time.totalRapidTime,
            "tool_change_seconds": time.totalToolChangeTime,
            "tool_change_count": time.toolChangeCount,
            "feed_distance_cm": time.feedDistance,
            "rapid_distance_cm": time.rapidDistance,
        }
        return {
            "metrics": metrics,
            "result": {"assumptions": payload},
    ...
    if action == "postprocess":
        ...
        return {
            "completed": not app.hasActiveJobs,
    ...
    if action == "collect_outputs":
        ...
        paths = sorted(
            p for p in Path(payload["output_directory"]).resolve().rglob("*") if p.is_file()
        )
        if not paths:
            raise RuntimeError("No output files produced")
        return {
            "result": {"files": [_artifact(p) for p in paths]},
    ...
    if action == "simulation":
        return {
            ...
            "issues": [
                {
                    "type": "needs_ui_verification",
                    "message": (
                        "No established milling verification results API. Capture completed "
                        "Fusion UI verification for this candidate and setup."
                    ),
                }
            ],
            "result": _inspect(app),
