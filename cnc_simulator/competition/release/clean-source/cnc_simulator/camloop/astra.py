...
STRATEGY = schema({
    ...
    "shortest_c": {"type":"boolean"}, "junction_cleanup": {"type":"boolean"}
})
...
SCHEMAS['drawing_intake']=schema({
 'name':{'type':'string'},'summary':{'type':'string'},
 'stock_kind':{'type':'string','enum':['box','cylinder']},
 'stock_dimensions_mm':{'type':'array','items':{'type':'number'}},
 'drawing_tolerance_mm':{'type':['number','null']},
 'blockers':{'type':'array','items':{'type':'string'}},
 'features':{'type':'array','items':schema({'name':{'type':'string'},'a':{'type':'array','items':{'type':'number'}},'b':{'type':'array','items':{'type':'number'}},'radius_mm':{'type':'number'},'floor_z_mm':{'type':'number'},'evidence_quote':{'type':'string'}})}
})
...
def worker(request_path, response_path):
    ...
    # Persist the exact supplied instruction layer before inference, not reconstructed later.
    Path(request_path).with_suffix('.instructions.txt').write_text(instructions)
    Path(request_path).with_suffix('.execution.json').write_text(json.dumps(dict(model=MODEL,service_tier='fast',reasoning_effort='low',role=role),indent=2))
    with Codex(config) as codex:
        ...
        result = thread.run(json.dumps(request["context"]), output_schema=SCHEMAS[role], effort=ReasoningEffort.low, service_tier="fast")
        ...
        Path(response_path).write_text(
            json.dumps(
                dict(
                    model=MODEL, requested_service_tier="fast", reasoning_effort="low", role=role, value=value, thread_id=str(thread.id), turn_id=result.id
                ),
                indent=2,
            )
        )
