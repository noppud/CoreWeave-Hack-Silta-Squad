...
def initialize_workspace(work, seed):
    work.mkdir(parents=True,exist_ok=True);(work/'runs').mkdir(exist_ok=True)
    if not (work/'memory.json').exists():
        initial=read(seed) if seed.exists() else dict(version=0,episodes=[],strategy_prior=None)
        save(work/'memory.json',initial);save(work/'memory/versions'/f"{initial['version']:04d}.json",initial)
    return read(work/'memory.json')

def main():
    import argparse
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--port',type=int,default=2744)
    args=parser.parse_args()
    if not 1<=args.port<=65535:parser.error('Port must be between 1 and 65535.')
    initialize_workspace(WORK, ROOT/'workspace-loop-demo/memory.json')
    ...
    server=ThreadingHTTPServer(('127.0.0.1',args.port),partial(Handler,directory=str(ROOT/'viewer/dist')));server.origin=f'http://127.0.0.1:{args.port}';server.serve_forever()
