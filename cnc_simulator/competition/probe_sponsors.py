...
try:
 auth=cwsandbox.AuthHeaders(headers={'x-wandb-api-key':key,'x-entity-id':'silta','x-project-name':'coreweave-hack-silta-squad'},strategy='wandb_api_key')
...
p=Path(__file__).with_name('sponsor-access.json');p.write_text(json.dumps(receipt,indent=2));print(json.dumps(receipt,indent=2))
