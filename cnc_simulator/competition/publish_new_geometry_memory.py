...
dataset=weave.Dataset(name='new-geometry-cam-memory-six-pairs',rows=rows);uri=weave.publish(dataset).uri();receipt=dict(dataset_uri=uri,protocol=data['protocol'],scope='Publication scores retained files from fresh 12-proposal experiment; this evaluation runtime is not simulator runtime.',evaluations=[])
for arm in ['cold','warm']:
 logger=weave.EvaluationLogger(name='new-geometry-cam-memory-'+arm,model={'name':arm,'proposal_model':'gpt-6-astra','memory_frozen':True},dataset=dataset,eval_attributes={'scope':receipt['scope']})
 for row in rows:
