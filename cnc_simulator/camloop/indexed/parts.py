...
def catalog():
    ...
    features=[top('Main gallery',[-13,0],[13,0],11,15),top('Control port',[0,16],[0,16],6,24)]
...
def prepare(root,count=10,parts=None):
    ...
    for part in (catalog()[:count] if parts is None else parts):
        ...
        # Remove sub-micron Boolean slivers before freezing, then verify the
        # actual serialized input rather than only the in-memory CAD.
        solid=manifold3d.Manifold(manifold3d.Mesh(np.asarray(target.vertices,np.float32),np.asarray(target.faces,np.uint32)))
        cleaned=solid.simplify(.001).to_mesh()
        target=trimesh.Trimesh(vertices=cleaned.vert_properties[:,:3],faces=cleaned.tri_verts,process=True)
        stock.export(directory/'stock.stl');target.export(directory/'target.stl')
        target=trimesh.load_mesh(directory/'target.stl',process=True)
        if not target.is_volume or not trimesh.load_mesh(directory/'stock.stl').is_volume:
            raise ValueError('Serialized CAD is not a closed solid: '+part['id'])
    ...
    save(root/'catalog.json',dict(parts=entries,objective='estimated machining seconds; geometry and configured collisions are pass/fail',machine='Haas UMC-750 Reboot; indexed 3+2',learning_split='Two development parts; eight held-out geometries unseen by learning prompts before guidance freeze' if parts is None else 'Custom supplied geometries; no controlled transfer split implied'))
    return entries
