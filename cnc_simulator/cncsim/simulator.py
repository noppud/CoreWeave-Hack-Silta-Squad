...
_GEOMETRY_CACHE = OrderedDict()  # Process-local, bounded, content-addressed immutable queries.
...
def simulate(plan, *, base_dir=".", output_dir=None, on_step=None, snapshot_stride=1):
    ...
    specs = [plan["target"], plan["stock"]] + plan["fixtures"]
    signatures = [{**spec, "sha256": hashlib.sha256((Path(base_dir)/spec["path"]).read_bytes()).hexdigest()} if spec["type"]=="mesh" else spec for spec in specs]
    cache_key = hashlib.sha256(json.dumps([signatures,pitch,shape,origin.tolist(),guard],sort_keys=True).encode()).hexdigest()
    cached = _GEOMETRY_CACHE.get(cache_key)
    if cached is None:
        masks = stock.occupancy(points, pitch, guard)
        distances = target.signed(points)
        fixture_masks = [f.occupancy(points,pitch,guard)[:2] for f in fixtures]
        for array in [*masks,distances,*[a for pair in fixture_masks for a in pair]]:
            array.flags.writeable = False
        cached = (masks,distances,fixture_masks)
        _GEOMETRY_CACHE[cache_key] = cached
        if len(_GEOMETRY_CACHE)>2:_GEOMETRY_CACHE.popitem(last=False)
    else:
        _GEOMETRY_CACHE.move_to_end(cache_key)
    lower, upper, nominal = (a.copy() for a in cached[0])
