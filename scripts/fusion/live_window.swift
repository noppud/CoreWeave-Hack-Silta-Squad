...
final class Frames: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    func snapshot() -> (Int, Double, String?) {
        lock.lock(); defer { lock.unlock() }
        return (count, lastFrame, failure)
    }
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sample: CMSampleBuffer,
        ...
        do {
            ...
            lock.lock(); count += 1; lastFrame = Date().timeIntervalSince1970; lock.unlock()
        } catch {
        ...
        do {
            ...
            guard windows.count == 1 else {
                throw NSError(domain: "Silta", code: 2,
                    userInfo: [NSLocalizedDescriptionKey: "Waiting for exactly one visible Fusion main window"])
            }
            ...
            while !FileManager.default.fileExists(atPath: directory.appendingPathComponent("stop").path)
                ...
                writeStatus([
                    ...
                    "fps_limit": 8, "source": "ScreenCaptureKit Fusion window",
