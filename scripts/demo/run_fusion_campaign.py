...
def main():
    ...
    if args.manifest.exists():
        ...
        campaign = dict(campaign_id=args.prefix, status='ready', current_part_id=None,
                        initial_versions=learning.active(),
                        learning_directory=str(learning.root), parts=[], events=[])
    for config in args.configs:
        ...
        row = rows[0] if rows else dict(part_id=job_id, job_id=job_id,
                 label=data.get('demo_part_spec', {}).get('name', config.stem),
                 config=str(config), manifest_path=str(path), status='queued')
        if not rows:
        ...
        command = [sys.executable, '-m', 'silta', 'run', str(config),
                   '--job-id', job_id, '--runs', str(ROOT/'runs'),
                   '--learning-directory', str(learning.root),
        ...
        row.update(status=actual.get('status', 'failed'), exit_code=result.returncode,
                   ending_versions=learning.active(), finished_at=time.time(),
