...
def _reference():
    return {
        ...
        "version_id": "urn:adsk.wipprod:fs.file:vf.abc?version=2",
        "version_number": 2,
        "document_name": "candidate-uuid",
        "project_id": "project",
    }
...
@pytest.mark.parametrize(
    "version",
    ["urn:adsk.wipprod:fs.file:vf.abc", "latest", "urn:adsk.wipprod:fs.file:vf.abc?version=3"],
)
...
def test_save_status_cannot_attach_another_active_document():
    with pytest.raises(RuntimeError, match="Active document changed"):
        save_status(
            SimpleNamespace(activeDocument=SimpleNamespace(name="other")),
...
def test_fork_cannot_reuse_parent_identity():
    document = SimpleNamespace(
        name="candidate-uuid", dataFile=SimpleNamespace(isComplete=True, id="parent")
    )
...
def test_open_rejects_dirty_copy_or_wrong_opened_version():
    ...
    data = SimpleNamespace(
        ...
        versionId=ref["version_id"],
        isComplete=True,
        parentProject=SimpleNamespace(id=ref["project_id"]),
    )
    ...
    app = SimpleNamespace(data=SimpleNamespace(findFileById=lambda _: data), documents=Documents())
...
def test_reopen_preserves_modified_copy_before_closing(tmp_path, monkeypatch, export_succeeds):
    ...
    app = SimpleNamespace(
        data=SimpleNamespace(findFileById=lambda _: data), documents=Documents([dirty])
    )
