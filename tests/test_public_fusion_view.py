...
def server(tmp_path):
    ...
    (tmp_path / "status.json").write_text(json.dumps({
        "status": "live", "heartbeat": time.time(), "frames": 12,
        "captured_at": time.time(), "viewport_only": True,
        "connection_url": "must never leak", "window_title": "private metadata",
    }))
    app = ThreadingHTTPServer(("127.0.0.1", 0), make_handler(
        tmp_path, tmp_path / "index.html", time.time() + 100
    ))
...
@pytest.mark.parametrize("path", ["/api/state", "/api/start", "/artifact/file.nc", "/live/connection", "/../.private/connection.json"])
...
@pytest.mark.parametrize("changes", [{"captured_at": 0}, {"viewport_only": False}])
def test_old_or_uncropped_picture_is_not_published(server, changes):
    path = server[1] / "status.json"
    data = json.loads(path.read_text())
    path.write_text(json.dumps({**data, **changes}))
    assert get(server, "/frame.jpg")[0] == 503
    assert get(server, "/stream.mjpg")[0] == 503


def test_stream_stops_when_capture_loses_viewport_marker(server):
    connection = http.client.HTTPConnection("127.0.0.1", server[0], timeout=2)
    ...
    path = server[1] / "status.json"
    data = json.loads(path.read_text())
    path.write_text(json.dumps({**data, "viewport_only": False}))
