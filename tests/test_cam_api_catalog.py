...
@pytest.mark.parametrize("payload", [{"setup_index": -1}, {"setup_index": True},
                                     {"strategies": "pocket2d"},
...
def test_introspection_creates_inputs_but_never_adds_operations(monkeypatch):
    ...
    value = SimpleNamespace(objectType="adsk::cam::ChoiceParameterValue", value="climb",
                            getChoices=lambda: (True, ["Climb"], ["climb"]))
    parameter = SimpleNamespace(name="actualDirection", title="Direction", value=value,
                                expression="'climb'", isEnabled=True, isEditable=True,
                                isDeprecated=False)
    operations = SimpleNamespace(count=0, add=Mock(), createInput=Mock(
        return_value=SimpleNamespace(parameters=[parameter])), compatibleStrategies=[
            SimpleNamespace(name="pocket2d", title="2D Pocket", isGenerationAllowed=True)])
    setup = SimpleNamespace(name="Silta", operations=operations, attributes=SimpleNamespace(
        itemByName=lambda *_: SimpleNamespace(value="v1")))
    ...
    app = SimpleNamespace(activeDocument=SimpleNamespace(products=SimpleNamespace(
        itemByProductType=lambda _: cam)))
