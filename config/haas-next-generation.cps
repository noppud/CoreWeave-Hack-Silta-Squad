...
var gFeedModeModal = createOutputVariable({onchange:function() {setFeedOutputFormat();}}, gFormat); // modal group 5 // G93-95
...
var gWorkplaneModal = createOutputVariable({current:255, onchange:function() {state.twpIsActive = gWorkplaneModal.getCurrent() == 254; machineSimulation({});}}, gFormat); // G254-G255
...
var settings = {
  ...
  allowCancelTCPBeforeRetracting: true // allows TCP/tool length compensation to be canceled prior retracting
};
...
function onOpen() {
  ...
  if (settings.workPlaneMethod.useTiltedWorkplane) {
    validate(getSetting("workPlaneMethod.useABCPrepositioning", true), localize("Setting 'useABCPrepositioning' must be enabled when 'useTiltedWorkplane' is enabled."));
...
var WARNING_USEPOLARMODE = 0;
function setPolarMode(section, mode) {
  if (usePolarMode && ((section.getPolarMode && section.getPolarMode() != POLAR_MODE_OFF))) {
    warningOnce("The ManualNC Action command \"USEPOLARMODE\" and the operation option \"Machining Type Polar\" cannot be used together. Please select either option to enable polar machining, but not both.", WARNING_USEPOLARMODE);
    usePolarMode = false; // do not enable polar mode manually when already done by the post kernel
    return;
  }
...
function onCommand(command) {
  ...
  case COMMAND_BREAK_CONTROL:
    if (!toolChecked) { // avoid duplicate COMMAND_BREAK_CONTROL
      ...
      toolChecked = true;
      lengthCompOutput.setCurrent(lengthCompCodes.cancel); // macro 9853 cancels tool length compensation
    }
    return;
...
function onFeedMode(mode) {
  ...
  case FEED_INVERSE_TIME:
    ...
    break;
  default:
    error(subst(localize("Invalid feedrate mode: %1"), mode));
  }
}

function setFeedOutputFormat() {
  if (gFeedModeModal.getCurrent() == 93) {
    feedOutput.setFormat(inverseTimeFormat);
  } else if (gFeedModeModal.getCurrent() == 95) {
    feedOutput.setFormat(fprFormat);
  } else {
    feedOutput.setFormat(fpmFormat);
  }
  feedOutput.setControl(gFeedModeModal.getCurrent() == 93 ? CONTROL_FORCE : CONTROL_CHANGED);
}

function onSectionEnd() {
  onFeedMode(FEED_PER_MINUTE); // set feed mode to FPM
...
function subprogramDefine(_initialPosition, _abc) {
  if (getSimulationStreamPath() != "" && getProperty("useSubroutines") != "none") {
    warning(subst(localize("Property '%1' is disabled during machine simulation."), properties.useSubroutines.title));
    setProperty("useSubroutines", "none");
  }
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    if (!getProperty("useG0") && (((x ? 1 : 0) + (y ? 1 : 0) + (z ? 1 : 0)) > 1)) {
      ...
      onFeedMode(currentSection.feedMode); // ensure correct feed mode is set for the operation
    } else {
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  writeStartBlocks(isRequired, function() {
    ...
    if (currentSection.isMultiAxis() && getSetting("workPlaneMethod.prepositionWithTWP", true) && getSetting("workPlaneMethod.useTiltedWorkplane", false) &&
      ...
      cancelWorkPlane();
      writeBlock(gMotionModal.format(0), getLengthCompCode(), hOffset, additionalCodes[1]); // G0 motion mode is required for the G234 command
      forceXYZ();
      ...
      if (machineConfiguration.isHeadConfiguration()) {
        writeBlock(modalCodes, gMotionModal.format(motionCode.multi), getLengthCompCode(),
...
// <<<<< INCLUDED FROM include_files/initialPositioning_haas.cpi
var lengthCompCodes = {tool:43, tcp:234, cancel:49};
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  var useG95 = currentSection.feedMode == FEED_PER_REVOLUTION || (getProperty("useG95forTapping") && isTappingCycle());
  if (forceCycle || isFirstCyclePoint()) {
    // return to initial Z which is clearance plane and set absolute mode
    repositionToCycleClearance(cycle, x, y, z);
    writeBlock(gFeedModeModal.format(useG95 ? 95 : 94));
    feedOutput.setFormat(useG95 ? fprFormat : feedFormat);
    ...
    case "counter-boring":
      ...
      break;
    case "chip-breaking":
      var useG73Retract = getProperty("useG73Retract", true);
      ...
      (useG73Retract && (cycle.accumulatedDepth < cycle.depth) && (cycle.incrementalDepthReduction > 0))) {
        ...
        writeBlock(
          ...
          P > 0 ? "P" + milliFormat.format(P) : "", // optional
          E, feedOutput.format(F)
        );
      } else {
        writeBlock(
        ...
        while (u > cycle.bottom) {
          ...
          if (first) {
            ...
            if (isPolarModeActive() || ((currentSection.getPolarMode && currentSection.getPolarMode() != POLAR_MODE_OFF) && currentSection.isMultiAxis())) {
              ...
              x = polarPosition.first.x;
              y = polarPosition.first.y;
              z = polarPosition.first.z;
            }
            ...
            writeBlock(u <= cycle.bottom ? gRetractModal.format(98) : "", position, depth);
          }
...
function getCommonCycle(x, y, z, r, c) {
  ...
  if (isPolarModeActive() || ((currentSection.getPolarMode && currentSection.getPolarMode() != POLAR_MODE_OFF) && currentSection.isMultiAxis())) {
    var polarPosition = getPolarPosition(x, y, z);
    return [xOutput.format(polarPosition.first.x),
